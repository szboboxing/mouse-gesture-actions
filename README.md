...
Windows 全局鼠标组合工具。只有按住鼠标右键时，滚轮和侧键才会执行自定义动作；松开右键后退出组合状态。未按右键时，普通左键及浏览器前进、后退侧键保持原生功能。

左侧“鼠标按键测试”模块可实时检测左键、右键、中键、滚轮方向及两枚侧键，并在鼠标图形上高亮对应区域。
...
| 侧键截图 | 右键按住 + `XButton1` 或 `XButton2` | 调用 `Win+Shift+S` |
...
- 右键按住时，`XButton1` 或 `XButton2` 均可立即截图。
- 未按右键时，两枚侧键完整透传给浏览器。
- 普通鼠标左键始终透传。
...
V1.0-V1.3 配置可继续读取，已移除的轨迹和组合窗口字段会被自动忽略。

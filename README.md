...
V2.1 增加主窗口关闭保护：关闭窗口时可选择确认关闭、最小化或取消，默认焦点和鼠标指针停留在取消按钮。
...
同一侧键最多启动一组映射；若两组选择相同侧键，后启动或后修改的一组生效，另一组自动停用。点选“增强粘贴”不会清空已选修饰键和主键，取消点选即可恢复原快捷键。配置和启用状态会立即保存。
...
点击主窗口右上角关闭按钮后，不会立即退出软件，而是显示三个选择：
...
V1.0-V1.9 配置可继续读取；旧配置默认启用两枚截图侧键，并生成两组初始关闭的键盘映射。已移除的轨迹和组合窗口字段会被自动忽略。
...
- [V2.2 版本说明（仅本地版本）](docs/releases/V2.2.md)

...
Windows 全局鼠标组合工具。V1.2 采用“方案 A”：只有按住鼠标右键时，滚轮和 `XButton1` 组合才会执行自定义动作；松开右键后退出组合状态。未触发组合的普通右键单击保持 Windows 原生菜单。
...
| 功能 | 触发方式 | 动作 |
|---|---|---|
| 复制 | 右键按住 + 滚轮向上 | 发送 `Ctrl+C`，复制选中的文本或文件 |
| 增强粘贴 | 右键按住 + 滚轮向下 | 新建文件夹，进入重命名状态并粘贴剪贴板内容 |
| 侧键截图 | 右键按住 + `XButton1` | 调用 `Win+Shift+S` |
| 滚轮组合截图 | 右键按住 + 上滚→下滚 | 在设定时间窗口内调用 `Win+Shift+S` |
...
- 单次下滚：立即执行增强粘贴。
- 上滚→下滚：仅在时间窗口内判定为截图。
- 下滚→上滚：只执行先发生的增强粘贴，不判定为截图。
- 上滚后超过窗口再下滚：执行先发生的复制。
- `XButton1`：在右键保持期间立即截图。

截图组合窗口可在 `200-300ms` 范围内设置，默认 `250ms`。滚轮刻度松散或存在抖动时，建议降低窗口；窗口越低，主动执行“上滚→下滚”所需速度越快。
...
V1.0/V1.1 配置可继续读取，已移除字段会被忽略，截图组合窗口采用默认 `250ms`。
...
```text
dist\鼠标手势动作小工具_V1.2.exe
```

## 权限与限制

- 普通权限程序不能向管理员权限窗口发送按键，目标程序以管理员身份运行时，本工具也需要相同权限。
- `XButton1` 需要鼠标硬件和驱动向 Windows 上报对应侧键消息。
- 滚轮组合识别依赖硬件事件顺序，滚轮抖动可能造成误触。
